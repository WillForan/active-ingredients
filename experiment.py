...
def extract_ingredients(soup):
    ...
    if 'currentSku' in prod_keys:
        ...
        return set([cleanup_ingredient(x) for x in BeautifulSoup(ingredients_html, 'html.parser').text.split(", ")])
    else:
        ...
        # TODO: replace all commas in each '(.*?)' with ';' ?
        # this is a nest list.
        ingredients_nested = [[cleanup_ingredient(y) for y in BeautifulSoup(x.get('ingredientDesc', ''), 'html.parser').text.split(", ")] for x in ingredients_per_type]
